...
#include <stdio.h>
#define DEBUG(fmt, ...)                                                        \
  fprintf(stderr, "scanner.c:%d DEBUG: " fmt, __LINE__, __VA_ARGS__)
#else
#define DEBUG(fmt, ...)
#endif
...
#define skip_hexdigits(lexer, maxlen)                                          \
  _skip_chars(lexer, maxlen, "0123456789ABCDEFabcdef")
...
unsigned int tree_sitter_perl_external_scanner_serialize(void *payload,
                                                         char *buffer) {
...
bool tree_sitter_perl_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  // The only time we'd ever be looking for both BEGIN and END is during an
  // error condition. Abort in that case
  if (valid_symbols[TOKEN_Q_STRING_BEGIN] &&
      valid_symbols[TOKEN_QUOTELIKE_END]) {
  ...
  /* We can't "un-advance" this backslash if TOKEN_ESCAPED_DELIMITER didn't want
   * it to leave it for TOKEN_QW_LIST_CONTENT, so we'll have to eat it now and
   * remember that we did so for all of them
   */
  if (begins_backslash && (valid_symbols[TOKEN_ESCAPE_SEQUENCE] ||
  ...
  if (valid_symbols[TOKEN_ESCAPE_SEQUENCE] && begins_backslash) {
    ...
    if (valid_symbols[TOKEN_QW_LIST_CONTENT]) {
      // Inside a qw() list, only \\ is a valid escape sequence; all else is
      // literal
  ...
  if (valid_symbols[TOKEN_Q_STRING_CONTENT] ||
      valid_symbols[TOKEN_QQ_STRING_CONTENT]) {
